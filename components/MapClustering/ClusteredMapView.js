...
const ClusteredMapView = ({
  ...
  useEffect(() => {
    if (locationSelected?.geometry?.location?.lat) {
      mapRef.current?.animateToRegion(
        {
          latitude: parseFloat(locationSelected?.geometry?.location?.lat),
          longitude: parseFloat(locationSelected?.geometry?.location?.lng),
          latitudeDelta: 0.01,
          longitudeDelta: 0.01,
        },
        500,
  ...
  const selectMerchant = value => {
    if (value && value?.store_lat) {
      ...
      mapRef.current?.fitToCoordinates(
        [
          {
            latitude: parseFloat(value.store_lat) - 0.000094,
