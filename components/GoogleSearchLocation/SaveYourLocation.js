...
const FLAT_LIST_TOP = WINDOW.height - 400 - 100;
...
export default class GooglePlacesAutocomplete extends Component {
  ...
  _onFocus = () => {
    this.setState({listViewDisplayed: true, isFocus: true});
  };

  _getFlatList = () => {
    const {keyboardHeight, dataSource, isFocus, listViewDisplayed} = this.state;
    const keyGenerator = () =>
      Math.random()
        .toString(36)
        .substr(2, 10);
    ...
    return (
      <FlatList
        ...
        style={[
          ...
          {top: isFocus ? FLAT_LIST_TOP - keyboardHeight : FLAT_LIST_TOP},
        ]}
